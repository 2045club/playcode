...
import { getProjectFileChangePreviews } from "@/lib/server/git-diff";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

type FileChangesRequestBody = {
  projectId?: number;
  paths?: string[];
};
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => ({}))) as FileChangesRequestBody;
  ...
  try {
    ...
    const files = await getProjectFileChangePreviews({
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "读取文件变更失败。",
      },
      { status: 400 },
    );
  }
